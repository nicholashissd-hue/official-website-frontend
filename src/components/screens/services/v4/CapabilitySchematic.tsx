...
/**
 * The sub-offering line has to fit inside its node, and only the browser knows
 * how wide a line of type actually is. Two of the eight run past the node
 * border at their natural width (08 by 19 units, 02 by 6), and a third clears
 * by 6, so a hand-tuned exception on the one that shows today would just move
 * the bug to whoever edits the copy next.
 *
 * The fix is what a draughtsman does with a label that runs long: tighten the
 * tracking rather than shrink the type or break the line. These are set with
 * 0.04em of added tracking, and on a 47-character line that added tracking is
 * almost exactly the overflow, so lengthAdjust="spacing" squeezes the gaps
 * back toward the mono's natural spacing and touches no glyph. Lines that fit
